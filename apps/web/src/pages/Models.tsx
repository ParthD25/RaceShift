        ...
        {artifacts.map(a => (
          <Panel key={a.id} title={a.label ?? a.id} icon={<Cpu size={17} />} action={<SourceBadge kind={a.ready ? sourceKindFor(a.is_synthetic) : 'offline'} text={a.role === 'baseline-report' ? 'Metrics only' : a.ready ? undefined : 'Incomplete'} />}>
            ...
            <div className="detail-list compact">
              ...
              {a.lap_validity_version != null && <div><span>Lap rules</span><strong className={a.validity_rules_match === false ? 'bad' : undefined}>v{a.lap_validity_version}{a.validity_rules_match === false ? ' · runtime applies a newer rule set, retrain before trusting its metrics' : ''}</strong></div>}
