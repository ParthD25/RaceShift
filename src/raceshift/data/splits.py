...
def season_round_split(df: pd.DataFrame, train_end: int, holdout_year: int, cutoff_round: int):
    ...
    train = df[df["season"] <= train_end].copy()
    val = df[(df["season"] == holdout_year) & (rounds <= cutoff_round)].copy()
...
def chronological_split(df: pd.DataFrame, train_end: int, val_year: int, test_year: int, split_round: int | None = None):
    ...
    if split_round is not None and val_year == test_year:
        return season_round_split(df, train_end, val_year, split_round)
...
def split_from_args(
    ...
    if holdout_event:
        ...
        return train, val, test
    return chronological_split(df, train_end, val_year, test_year, split_round=split_round)
