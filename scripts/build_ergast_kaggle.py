...
"""Build RaceShift legacy-tier lap tables from the Kaggle Ergast CSV dump.
    ...
    python scripts/build_ergast_kaggle.py --csv-dir data/raw/ergast_kaggle --years 1996-2026 \
...
produces from the API, so either can feed the legacy-extended training table.
"""
...
def main() -> None:
    ...
    args = p.parse_args()
    path = export_seasons(args.csv_dir, parse_years(args.years), args.output)
    print(path)
