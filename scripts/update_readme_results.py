...
def table(summary: dict, title: str) -> list[str]:
    ...
    lines.append("| Model | Test MAE (s) | Test RMSE (s) | p90 (s) | 80% coverage | Train time (s) | Peak RSS (MB) | Artifact (MB) |")
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
    ...
    for r in summary["results"]:
        ...
        lines.append(
            ...
            f"{fmt(tr.get('peak_rss_mb'), 0) if tr.get('peak_rss_mb') else '—'} | {fmt(art / 1e6, 2) if art else '—'} |"
        )
    lines.append("")
    return lines
