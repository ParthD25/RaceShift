...
from raceshift.data.tracinginsights_loader import SESSION_FOLDERS, apply_calendar, raw_url, session_frame  # noqa: E402
from fetch_fastf1_seasons import parse_years  # noqa: E402


def list_session_files(year: int, cache: Path, warnings: list[str] | None = None) -> list[tuple[str, str]]:
    """(event, session folder) pairs that have a session_laptimes.json, from a partial clone.

    A clone that cannot be refreshed is still listed, and the staleness is reported in
    ``warnings`` so a run that used an old index is not mistaken for a complete one."""
    ...
    if not (repo / ".git").exists():
        ...
        fetched = subprocess.run(["git", "-C", str(repo), "fetch", "-q", "--depth", "1", "origin"], capture_output=True, text=True)
        if fetched.returncode == 0:
            subprocess.run(["git", "-C", str(repo), "reset", "-q", "--soft", "origin/HEAD"], check=True)
        else:
            note = f"{year}: could not refresh the season index ({fetched.stderr.strip() or 'git fetch failed'}); listing the cached index, which may lack recent sessions"
            print("WARNING " + note, flush=True)
            if warnings is not None:
                warnings.append(note)
    # NUL-separated output: git would otherwise quote and escape paths with non-ASCII
    # characters (São Paulo Grand Prix), which then fail to fetch.
    listing = subprocess.run(["git", "-C", str(repo), "ls-tree", "-r", "-z", "--name-only", "HEAD"], check=True, capture_output=True, text=True).stdout
    out = []
...
def main() -> None:
    ...
    p.add_argument("--sessions", nargs="+", default=["R"], type=str.upper, choices=sorted(SESSION_FOLDERS.values()), help="Session codes: R, S, Q, SQ, SS, FP1-3")
    ...
    p.add_argument("--rounds-from", help="Lap table (parquet/csv with season, event, round_number, circuit, event_date), e.g. the FastF1 table, giving the official round numbers and the circuit and event date the FastF1 tier uses; without it round_number is left unset (event_date still orders the events)")
    args = p.parse_args()
    ...
    failures: list[str] = []
    warnings: list[str] = []
    if not args.rounds_from:
        warnings.append("no --rounds-from table: round_number is left unset; pass the FastF1 or Ergast table for official round numbers")
    for year in parse_years(args.years):
        for event, folder in list_session_files(year, cache, warnings):
            ...
            if not json_cache.exists():
                ...
                for attempt in range(4):
                    ...
                    failures.append(f"{year} {event} {folder}: {outcome}")
                    continue
            if not json_cache.exists():
    ...
    calendar = None
    if args.rounds_from:
        calendar = pd.read_parquet(args.rounds_from) if Path(args.rounds_from).suffix.lower() == ".parquet" else pd.read_csv(args.rounds_from)
    frames = apply_calendar(frames, calendar)
    for frame in frames:
        target = output / f"{int(frame['season'].iloc[0])}_{str(frame['event'].iloc[0]).replace(' ', '_')}_{frame['session'].iloc[0]}.parquet"
        frame.to_parquet(target, index=False)
    print(f"Sessions: {len(frames)}; failures: {len(failures)}; warnings: {len(warnings)}")
